...
studies <- c("SDY56, SDY61, SDY63, SDY67, SDY80, SDY180, SDY212, SDY224, SDY269, SDY270, SDY400, SDY404, SDY520, SDY640, SDY984, SDY1119, SDY1260, SDY1264, SDY1276, SDY1289, SDY1291, SDY1293, SDY1294, SDY1325, SDY1328, SDY1364, SDY1368, SDY1370, SDY1373")
...
immdataFiltered$young <- filterImmdataByAge(immdata_all, ageCutoffs, isYoung = TRUE)
immdataFiltered$older <- filterImmdataByAge(immdata_all, ageCutoffs, isYoung = FALSE)
```


```{r generate-response-calls}
# Need age-cutoffs here for HAI and Nab to assign names to sub-study
...
# Reduce memory usage
rm(immdataWithResponses, immdataFiltered)
```
...
# Reduce memory usage
rm(allGE, summarizedEsets, esets, geMetaData, sharedMetaData, 
   phenoDataSets, demographics, gs, geneExpressionFiles)
gc()
```
...
esets$noResponse$noNorm$young <- noNormEset[ , noNormEset$age_imputed < ageCutoffs[[1]] ]
esets$noResponse$noNorm$older <- noNormEset[ , noNormEset$age_imputed >= ageCutoffs[[2]] ]

# Because of allGE merge, cohorts may have allNArows pre-norm
esets$noResponse$noNorm <- lapply(esets$noResponse$noNorm, function(eset){
...
# Use 'For' to minimize memory usage with cross-study norm work
# and batch correction requiring lmFit
esets$noResponse$norm <- list()
for(ageCohort in names(esets$noResponse$noNorm)){
  eset <- esets$noResponse$noNorm[[ageCohort]]
  eset <- crossStudyNormalize(eset, 
                              vendorToUse = "Affymetrix", 
                              studiesToExclude = "SDY1293")
  eset <- batchCorrectBaselineData(eset)
  eset <- removeAllNArows(eset)
  esets$noResponse$norm[[ageCohort]] <- eset
}
```
...
```{r output}
saveRDS(immdata_all, file = paste0(prefix, "IS2_immdata.rds"))
saveRDS(selectedImmdata, file = paste0(prefix, "IS2_immdata_selected.rds"))

for(responseStatus in names(esets)){
  for(normStatus in names(esets[[responseStatus]])){
    for(ageCohort in names(esets[[responseStatus]][[normStatus]])){
      eset <- esets[[responseStatus]][[normStatus]][[ageCohort]]
      name <- paste("IS2_eset", responseStatus, normStatus, ageCohort, sep = "_")
      saveRDS(eset, file = paste0(prefix, name, ".rds"))
    }
  }
}

IS2_session_info <- sessionInfo()
saveRDS(IS2_session_info, file = paste0(prefix, "IS2_session_info.rds"))
```
