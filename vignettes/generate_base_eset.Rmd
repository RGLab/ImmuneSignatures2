...
timeStamp <- "2020_12_07_PostAnno_"
```
...
# con <- CreateConnection("IS2", onTest = TRUE)
con <- CreateConnection("", onTest = TRUE) # TODO: rm once IS2 virtual study finalized
```
...
immdata_all <- sapply(assays, USE.NAMES = TRUE, function(assay){
  ...
  # dt <- dt[ , lsid := NULL ] # artifact of import into virtual study container
  if(assay == "elisa"){
...
esetsFile <- list.files(path = dataCacheDir, pattern = "IS2_esets.rds", full.names = TRUE)
