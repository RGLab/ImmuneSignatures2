...
### Figure 1d: Plot number of samples as a function of day of sampling
*Author: * Slim Fourati
```{r, fig-1d-data-prep}
...
```{r, check-cached-slea}
  cacheFile <- "data_cache/sleaSet.rds"
  sleaCached <- file.exists(cacheFile)
  if(sleaCached){
    sleaSet <- readRDS(cacheFile)
  }
```
...
saveRDS(sleaSet, file = "data_cache/sleaSet.rds")
```
...
*Author: * Slim Fourati
```{r, fig-s2a}
clusGapCacheFile <- "data_cache/clusGap.rds"
if(!file.exists(clusGapCacheFile)){
  set.seed(seed = 23)
  fit <- clusGap(t(zscoreTemp), kmeans, K.max = 10)
  saveRDS(fit, file = clusGapCacheFile)
}else{
  fit <- readRDS(clusGapCacheFile)
}
