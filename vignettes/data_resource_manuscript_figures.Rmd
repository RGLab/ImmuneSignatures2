...
datePrefix <- "2020_12_07_PostAnno"
```
...
```{r, read-esets}
noRespGEFile <- file.path(dataCacheDir, paste0(datePrefix,"_extendedOld_norm_noResponse_eset.rds"))
old.NoResp <- readRDS(file = noRespGEFile)

noRespGEFile <- file.path(dataCacheDir, paste0(datePrefix,"_young_norm_noResponse_eset.rds"))
young.NoResp <- readRDS(file = noRespGEFile)

withRespGEFile <- file.path(dataCacheDir, paste0(datePrefix,"_young_norm_withResponse_eset.rds"))
...
### Figure 1a: Proportion of subjects of various self reported race and imputed y chrom values per pathogen*vaccine type
*Authors:* Joanna Arce
...
sex_counts_df <- getCounts(pdata_df_uniqueSubjects, "gender_imputed", "SEX_")
...
p <- ggplot(total_counts) +
  ...
  scale_y_discrete(limits = rev(c("SEX_Male", 
                                  "SEX_Female",
                                  "RACE_White", 
