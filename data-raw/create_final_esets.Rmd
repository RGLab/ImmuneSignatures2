...
outputFilenamePrefix <- "test_batch_correction_"
...
for(ageGroupName in names(ageGroups)){
  ...
  # when ageGroup = "all", need to include age_group in covariates
  # Re-run with updates
  # Add age group variable, make sure that "young" comes first in factor levels
  # Re-do PCA for: 
  # old (batch corrected separately)
  # all (subsetted by young vs old)
  # Why are we not just doing everything together then subsetting? 
  # TODO: Also add age imputation (from Jeremy) and label as age_imputed_byGE
  if(grepl("old", ageGroupName, ignore.case = TRUE)){
    ...
    if (ageGroupName == "all") {
      eset.corr.noResponse_nocelltype <- batchCorrect(
        eset.norm.noResponse, 
        batch.vars = c('featureSetVendor',
                       'featureSetName2',
                       'geBatchName'),
        covariates = c('y_chrom_present',
                       'age_group',
      ...
      eset.corr.noResponse_nocelltype <- removeAllNArows(eset.corr.noResponse_nocelltype)
      eset.corr.noResponse_nocelltype <-
        eset.corr.noResponse_nocelltype[complete.cases(exprs(eset.corr.noResponse_nocelltype)), ]
      fileSuffix <- paste0(ageGroupName, "_norm_nocelltype_noResponse_eset.rds")
      filename <- file.path(dataCacheDir, paste0(outputFilenamePrefix, fileSuffix))
      saveRDS(eset.corr.noResponse_nocelltype, filename)
      # TODO: Make PCA from eset.corr.noResponse_nocelltype, only for baseline. 
    }
