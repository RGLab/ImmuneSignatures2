...
selectedImmdata <- lapply(immdata, function(immdata_age_group){
...
if( !results$hasNames || length(results$NAvalues > 0) || length(results$badDims > 0) ){
...
if (!all.equal(geMetaData$biosample_accession, colnames(allGE)[-length(colnames(allGE))])) {
    stop("biosample accessions do NOT match for expression data and meta-data!")
}
colnames(allGE) <- c(geMetaData$uid, "rn")
```
